...
var fns = [
  checkParams,
  checkClient,
  checkUserApproved, //TODO: Implement Consent
  generateCode,
  saveAuthCode,
  redirect
];
...
function saveAuthCode(done) {
  ...
  var redirectUri = this.req.param('redirect_uri');
  var scope = this.req.param('scope');
  var responseType = this.req.param('response_type');
  var nonce = this.req.param('nonce');

  this.model.saveAuthCode(this.authCode, this.client.clientId, expires,
    this.user, scope, this.user, redirectUri, responseType, 'created', nonce, function (err) {
      if (err) return done(error('server_error', false, err));
      done();
    });
}
