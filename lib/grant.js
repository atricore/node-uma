...
function saveAccessToken(done) {
  ...
  // OIDC-specific: Create ID Token and include it in the access token
  if (this.req.authCode) {
    var d = Math.round(new Date().getTime() / 1000);
    var idToken = {
      iss: this.req.protocol + '://' + this.req.headers.host,
      sub: this.req.authCode.sub.username || this.req.authCode.user.username || null,
      aud: this.client.clientId,
      exp: d + 3600,
      iat: d,
      nonce: this.req.authCode.nonce
    };

    this.idToken = jwt.encode(idToken, this.client.clientSecret);

    this.model.saveAccessTokenWithIDToken(accessToken, this.client.clientId, expires,
      this.user, 'Bearer', idToken,
      this.req.authCode.scope, this.req.authCode.code, this.req.authCode.nonce,
      function (err) {
        if (err) return done(error('server_error', false, err));
        done();
      });
...
function sendResponse(done) {
  ...
  if (this.idToken) response.id_token = this.idToken;

  if (this.req.authCode) response.code = this.req.authCode.code;

  if (this.req.state) resp.state = this.req.state;
