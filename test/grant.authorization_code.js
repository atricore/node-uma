...
describe('Granting with authorization_code grant type', function () {
  ...
  it('should allow valid request', function (done) {
    var app = bootstrap({
      model: {
        getClient: function (id, secret, callback) {
          callback(false, {client_id: 'thom', client_secret: 'secret'});
        },
        grantTypeAllowed: function (clientId, grantType, callback) {
          callback(false, true);
        },
        getAuthCode: function (refreshToken, callback) {
          ...
          callback(false, {
            ...
            user: {'id': '123', 'username': '123'},
            sub: {'id': '123', 'username': '123'},
            code: 'xyz123',
        ...
        saveAccessTokenWithIDToken: function (token, clientId, expires, user, type, idToken, scope, auth, nonce, cb) {
          cb();
        },
